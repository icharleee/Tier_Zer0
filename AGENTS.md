...
- Implementation proceeds in **vertical slices** (ADR-0015), each a complete constitutional loop through ontology, schema, API, database, UI, audit, and tests. Before writing a slice's code, its entities' Invariant Matrix rows and ERD coverage must exist. The success metric: a synthetic EvidenceArtifact travels the entire constitutional system without violating a single invariant.
