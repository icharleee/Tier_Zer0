...
- **Documentation Freeze is in effect** (ADR-0015 / ONT-PRN-011): no new foundational ADR or governance document unless implementation discovers a genuine gap or contradiction. When it does, follow the discovery flow — Ontology → Implementation → Discovery → ADR → Supersession. Implementation discovers missing ontology; it never defines meaning. Do not polish governed documents while slices are unfinished.
- Implementation proceeds in **vertical slices** (ADR-0015), each a complete constitutional loop through ontology, schema, API, database, UI, audit, and tests. Before writing a slice's code, its entities' Invariant Matrix rows and ERD coverage must exist. Slice 1 is **Constitutional Evidence Activation** (ADR-0007 §9) — the feature is activation, not upload. The success metric: a synthetic EvidenceArtifact travels the entire constitutional system without violating a single invariant.
- Every constitutional transition is explicit (ONT-PRN-012 / Resolution 007): implement lifecycle movement only as named operations that record actor, timestamp, and allowed predecessor state and emit the audit event in the same transaction — never as bare status-field writes. CI reports **Constitutional Coverage** (per-article test coverage via ONT identifiers) alongside code coverage (ADR-0007 §8).
