...
**Standing limitations (publication mandatory, per Sessions 012–013; status updated Sessions 019 and 021):** no evidentiary sufficiency model; no formal comparative assessment; no calibrated confidence; no AI provenance at Hypothesis; no end-user review workflow; and — added by Session 013 — no formal mechanism yet distinguishes creation admissibility from later human endorsement or institutional adoption (a warning that future workflow must never smuggle epistemic truth into administrative approval, not permission to build an "accepted hypothesis" state). ~~No authenticated actor identity~~ was **addressed at the constitutional identity layer** by Slice 1F-A (ONT-PRN-028); the authority and visibility constitutional models are **established** by Slice 1F-B (ONT-PRN-029/030). Remaining **operational implementation gaps** (not constitutional gaps): production identity-provider integration; persisted authority-grant administration.
...
Promotion despite n = 1 rests on the demonstrated **existence of failure classes local slices cannot express** — privileged placement, undocumented ordering, asymmetric schema, silent omission, visibility ambiguity, aggregation drift, composition-created narrative: every local validator could remain perfectly correct while the global view violated the Constitution. Status (updated by AGC Sessions 017 and 021): **supported across three distinct integration surfaces — global read-model composition (Slice 3A: locally safe epistemic records, unsafe composition remains possible), storage reconciliation (Slice 1E: locally passing persistence behavior, cross-representation inconsistency remains possible), and transport/API transaction interaction (Slice 1F-B: locally correct transaction handling, cross-connection lock deadlock remains possible).** Same deeper result on all three: local correctness does not guarantee global constitutional integrity. Retained as an ODE observation, not elevated to a universal principle; presentation composition (1F-C) is the next domain.

> **Observation O10 (promoted from candidate at AGC Session 019):** Mature constitutional infrastructure reduces later implementation divergence when new capabilities reuse previously proven semantics and enforcement surfaces instead of recreating equivalent mechanisms.

Evidence now spans two distinct integrations: Slice 3A reused the chain_version=1 canonical serializers (the predicted timestamp/JSON divergence hotspot disappeared into infrastructure already parity-tested in Slice 1B); Slice 1F-A preserved the audit-chain function byte-for-byte and added identity enforcement *around* it via a defensive trigger rather than rewriting a parity-proven invariant. Kept as an observation, not a universal principle. Also recorded as evidence for Principle P1 (architectural compounding: earlier rigor → reusable invariant → later integration cost falls).
...
> **Hypothesis H10 (refined at the Slice 1F-A gate review, AGC Session 018):** Independent constitutional command paths can derive actor attribution from authenticated principal context while refusing caller-controlled identity substitution and preserving the separation between authentication, actor attribution, authority, and epistemic meaning.

Registered ahead of Slice 1F-A (*Authenticated Actor Context*) — the first of the three 1F sub-slices Session 017 decomposed (identity → authority → presentation; H11 *authority and visibility* and H12 *the visible review surface* are reserved for the later sub-slices as separate experiments). Governed by ONT-PRN-028 (Resolution 023 / ADR-0033): identity is authenticated, never asserted; authentication establishes attribution only. The Session 018 refinement sharpens the subject from "API paths" to **constitutional command execution across multiple paths** and makes the four-way separation explicit (authentication ≠ attribution ≠ authority ≠ epistemic meaning). Honest bound, as with H9's probing: authentication is a declared single-implementation transport surface; the triangulated pair is the binding rule and the database consistency guard.

**Status: Supported (n = 1)** — Slice 1F-A, verdict confirmed by AGC Session 019: transport-derived principal identity; identity-free command schemas; caller identity fields refused even when they match; pure domain actor binding; transaction-scoped `SET LOCAL`; database mismatch refusal; no principal leakage across transactions; the structural absence of authority fields from `AuthenticatedPrincipal`; epistemically identical records across different authenticated authors; existing internal mutation paths unchanged. ARGUS now knows who is acting without allowing the caller to define who they are. The honest limitation stays attached: authentication itself has only one implementation here — the agreement tested is between the identity *layers* after authentication, not between multiple credential verifiers.

> **Observation O13 (promoted from candidate at AGC Session 019):** Identity integrity is strengthened when authentication evidence, domain attribution, and persistence enforcement remain distinct representations whose agreement is required at execution time.

The architecture demonstrated the separation — `PrincipalVerifier` establishes authenticated identity, `bind_actor` derives domain attribution, `assert_transaction_principal` checks persistence consistency — and the experiment proved two failure classes neither layer catches alone: caller identity assertion refused before the domain action, and bound-principal/actor mismatch refused at the persistence boundary. Status: **supported by the first authenticated-actor experiment (n = 1)**; a future production identity-provider integration is the useful repeat test.

*Candidate observation O14 (registered at AGC Session 019, deliberately not promoted — SEALED access under 1F-B is its first test):* Visibility can vary by principal without creating multiple epistemic realities when withholding is represented explicitly and the underlying constitutional record remains singular.

> **Hypothesis H11 (proposed at AGC Session 019, ahead of Slice 1F-B):** Independent authority-enforcement paths can permit or deny actions and visibility according to authenticated principal context and explicit resource-scoped capabilities while preserving epistemic neutrality, explicit withholding, and auditable protected access.

Registered ahead of Slice 1F-B (*Authority and Visibility*) — the second 1F sub-slice. Governed by ONT-PRN-029 (Resolution 024: authority governs permitted actions and visibility, never epistemic standing) and ONT-PRN-030 (Resolution 025: access to protected information is explicit, least-privileged, attributable, and auditable; absence of access is never absence of evidence). The central constraint: *a privileged user may see or do more; their privilege must never make the underlying record more true.* Action authority and visibility authority are kept distinct (they diverge — one may view SEALED metadata but not content, create an Observation but not dispose a Contradiction). H12 (*the visible review surface*) remains reserved for 1F-C.

The Session 020 refinement sharpens the wording to **authorized withholding** (withholding is exposed only when record existence may be disclosed — secrecy itself must not become an existence leak) and makes the honest bound explicit: the triangulated pair is the `authorize` decision (Python + PostgreSQL over the same normalized grant facts); the `AuthorityProvider` is a declared single-implementation grant-fact source, and protected-content enforcement is application-layer (content lives outside the database). **Refined statement:** *Independent authority-decision implementations can derive resource-scoped action and visibility permissions from authenticated principal context and normalized capability grants while preserving epistemic neutrality, explicit authorized withholding, least privilege, and attributable protected access.* **Status: Untested.**

*Candidate observation O15 (registered at AGC Session 020; first supporting evidence recorded at Session 021, deliberately not promoted):* Least-privilege authority is more stable when authorization decisions are derived from normalized capabilities and explicit resource scopes rather than organizational roles or implicit privilege inheritance. Slice 1F-B provides the first evidence, but the current provider is deliberately simple; the harder test — where capability models often collapse back into implicit role inheritance — comes with persisted grants, organizational membership, delegation, revocation, and multi-case or tenant scope. Promotion waits for a second authority source or persisted grant model. *(Designated repeat test: workstream 4A-3, persisted authority grants — AGC Session 024.)*

**Status of H11: Supported (n = 1)** — Slice 1F-B, verdict confirmed by AGC Session 021: Python/PostgreSQL authorization parity; grant facts separated from decisions; Case-scoped authority with no capability inheritance; authentication alone granting nothing; trusted-internal execution not satisfying user-facing authority; explicit existence protection and explicit withholding after existence authorization; protected access audited before disclosure, with audit failure preventing disclosure; `SEALED_VERIFY` distinct from `SEALED_CONTENT_READ` and non-epistemic; denied writes producing no mutation. The system maintained three separate truths at once: *one constitutional record, different authorized projections, identical shared epistemic meaning.* Qualification attached: the AuthorityProvider has one implementation and grants are not persisted — H11 establishes the decision and enforcement model, not production-grade grant administration. A load-bearing refinement to carry into future read-model documentation: *evidence mutation ≠ access-history mutation* — a protected read remains epistemically non-mutating even though it legitimately appends to audit history.

> **Observation O14 (promoted from candidate at AGC Session 021):** Principal-specific visibility can produce different authorized projections without producing different epistemic realities when the underlying constitutional record remains singular and withholding is represented explicitly.

The experiment demonstrated the required relation exactly: Principal A projection ≠ Principal B projection; underlying record = same constitutional record; shared epistemic fields = same values. Status: **supported by the first authority-and-visibility experiment (n = 1)**; a persisted grant provider or second presentation surface strengthens it further.

> **The locking discovery (formal record, AGC Session 021).** The H11 fixture hung the suite: a fixture transaction held the audit-head `FOR UPDATE` lock (`seal_artifact` does not commit on the PostgreSQL path) while the transport TestClient — a separate connection — blocked forever on the same lock. Local tests never crossed the connection boundary, so they never exposed the assumption. This is **O9's third distinct integration surface** (after global read-model composition and storage reconciliation): *transport/API transaction interaction* — local guarantees do not automatically survive composition, now substantially strengthened. It also strengthens candidate O12 (integration constraints as discovery instruments), retained as candidate for one more cycle.

*Candidate observation O16 (registered at AGC Session 021, deliberately not promoted — 1F-C tests it):* Presentation is an epistemically consequential layer even when it is operationally read-only, because ordering, prominence, language, color, and interaction defaults can create meaning absent from the underlying constitutional graph.

> **Hypothesis H12 (proposed at AGC Session 021, ahead of Slice 1F-C):** A human-facing review surface can present an authorized constitutional Case projection while preserving plurality, temporal truth, explicit boundaries, visibility constraints, and non-preference — without visual hierarchy, labels, ordering, or interaction defaults constructing a privileged narrative.

Registered ahead of Slice 1F-C (*Visible Review Surface*) — the third and final 1F sub-slice, and the third major O9 composition domain (data composition → storage composition → **presentation composition**). Governed by ONT-PRN-031 (Resolution 026 / ADR-0036): presentation may organize constitutional information for comprehension, but it must not create epistemic hierarchy, certainty, preference, or conclusion that does not already exist in the underlying authorized projection — and visual prominence, ordering, color, default expansion, interaction affordances, and labels are part of the constitutional presentation surface whenever they can imply evidentiary or epistemic meaning. The O9 falsifier is simple: can all lower layers remain constitutional while the interface alone introduces preference? Constitutional presentation includes accessibility semantics — a visually symmetric page can still be asymmetric for screen-reader or keyboard users. Review-workflow mutation is NOT AUTHORIZED for H12 (presentation first; workflow behind a later gate).

**Refined at the Slice 1F-C gate review (AGC Session 022):** *A human-facing presentation can render an authorized constitutional Case projection while preserving completeness, plurality, temporal truth, explicit boundaries, visibility constraints, accessibility-relevant semantics, and peer non-preference — without labels, ordering, prominence, color, grouping, or interaction defaults introducing new epistemic meaning or a privileged narrative.* The refinement adds the decisive clause — *presentation must not introduce new epistemic meaning* — and the honest bounds: the renderer is a declared single implementation downstream of the authorized projection (it holds no grants and calls no authorize); static DOM inspection verifies **accessibility-semantic source parity**, never full assistive-technology conformance; and H12 tests both content-parity directions (every projection value exactly once; nothing visible beyond the closed presentation vocabulary — repetition must not become prominence). The red-team obligation applies the O12 discipline to presentation: synthetic violating fixtures must fail the conformance scanner.

**Status: Supported (n = 1)** — Slice 1F-C, verdict confirmed by AGC Session 023: presentation downstream of authorization with no authority logic reachable from the renderer; full expansion, citation ordering and identity, equal component rules; visible retracted history; explicit historical/current distinctions; boundaries as relationships rather than warnings; neutral textual health definitions; SEALED withholding preserved; no existence disclosure before authorization; identical epistemic markup across differently-authorized projections; no workflow or endorsement controls; and detection of all eight planted presentation violations. The page can state what a record says, its citation, its retraction, what bounded it, its derived health, what was true at creation and now, and what is withheld; it cannot state which explanation is strongest, which interpretation deserves belief, which Hypothesis is probably correct, what the Case "really means," or what the reviewer should conclude. The limitation stays attached: H12 establishes **static server-rendered presentation conformance and accessibility-relevant source semantics** — not browser, CSS-rendering, keyboard-runtime, or assistive-technology conformance.

> **Observation O16 (promoted from candidate at AGC Session 023):** Presentation is epistemically consequential even when operationally read-only, because structure, repetition, ordering, prominence, language, color, and interaction defaults can introduce meaning or preference absent from the underlying constitutional records.

The strongest evidence is the red-team suite: each violating fixture changed no record, no projection, and no epistemic object — yet each created an unconstitutional presentation (featured placement, primary language, health-based ordering, asymmetric expansion, hidden retraction, truth-semantic styling, evaluative synthesis, repetition as prominence), and the scanner made those failures mechanically observable. Status: **supported by the first constitutional-presentation experiment (n = 1).**

> **Observation O12 (promoted from candidate at AGC Session 023):** Integration constraints can serve as discovery instruments by making failure classes expressible that narrower component tests cannot observe.

Supporting cases across multiple integration surfaces: reconstruction constraints exposing compositional risks; storage reconciliation exposing the persisted storage_ref defect; transport tests exposing the audit-head lock assumption; presentation conformance detecting eight planted failures ordinary rendering would permit — the last an especially clean demonstration, identifying entire categories of unconstitutional UI design rather than accidental bugs. Status: **supported across multiple integration surfaces.**

**O9 evidence annotation (Session 023):** supported across **four distinct composition surfaces** — read-model composition, storage reconciliation, transport/transaction composition, and human presentation composition. Retained as an ODE observation; its evidence is broad within ARGUS but remains one program and one architectural family — not promoted to universal law.

**The program-completion ledger (AGC Session 023).** The original slice roadmap is complete; H1–H12 stand Supported at n = 1 after their respective reviews. That does not mean the hypotheses are universally proven — it means each survived its first controlled implementation experiment within ARGUS. **The next research phase focuses on replication, not new hypotheses:** the strongest next evidence comes from changing environmental conditions while preserving the same constitutional derivations (another persistence implementation, a MinIO ContentStore adapter, a production identity provider, a persisted authority provider, a second presentation client, browser-driven accessibility testing, an independent implementation team). Replication now has greater research value than extending the ontology. The program phase changes from *ontology and constitutional construction* to **constitutional hardening and replication** (ADR-0037), with four registered Phase 4 hypotheses:

> **Hypothesis H13 (registered at AGC Session 023; refined at the Session 024 direction review):** Independently implemented infrastructure adapters can replace reference development implementations while preserving the applicable constitutional observations, derived states, refusal behavior, non-effects, visibility semantics, and epistemic neutrality defined by Constitutional Baseline 1.0.

The Session 024 refinement prevents H13 from collapsing into *both implementations satisfy the same programming interface* — interface compatibility is necessary, never sufficient (Resolution 027 / ONT-PRN-032 / ADR-0038). H13 is an **umbrella hypothesis with cumulative per-adapter evidence**; a single combined experiment was rejected because the three adapters replace different trust boundaries (storage-semantics drift, identity-attribution drift, permission-and-scope drift) and a combined failure would be causally uninterpretable. Sub-results accumulate in required order, each behind its own gate:

- **H13-S1 — Storage adapter replication (4A-1, MinIO):** a second ContentStore implementation preserves the storage and reconciliation semantics defined by Baseline 1.0 without changing integrity classification or epistemic meaning. **The designated O11 repeat test:** the second backend expresses failure through HTTP statuses, timeouts, access denial, and partial reads rather than filesystem exceptions — those facts must be normalized into the same observation vocabulary *before* classification, or O11 weakens. A success is reported as *O11 strengthened by a second storage backend*, not as a new n. The decisive vocabulary distinction: *not found* is never conflated with *not observable because access failed*.
- **H13-S2 — Identity adapter replication (4A-2, production PrincipalVerifier):** a second verifier produces the same normalized `AuthenticatedPrincipal` facts and preserves the same identity-substitution refusals. Primary falsifiers: two credentials for the same external account must normalize consistently; two distinct external accounts must never collapse into one principal; no authority, trust level, Case access, or risk facts may enter the principal (the 1F-A boundary).
- **H13-S3 — Authority adapter replication (4A-3, persisted grants):** durable grant state feeds the existing `authorize()` derivation without becoming a second decision engine — no inheritance, no wildcard, no role titles in the constitutional decision function. **The designated O15 repeat test:** if persisted grants preserve the exact-capability model, O15 becomes promotion-eligible; if operational pressure pulls roles or inheritance into the decision path, O15 is falsified or refined.

Every adapter gate classifies failure into the Session 024 taxonomy — contract failure; **conformance-suite insufficiency** (adapter passes the contract suite yet system behavior diverges from Baseline 1.0 — the most scientifically valuable H13 failure, answered by strengthening the contract and retesting *both* implementations, never by special-casing the new adapter); environmental divergence (the backend cannot provide equivalent semantics — the contract is never weakened to accommodate it); and **constitutional divergence** (every call "works" but a constitutional distinction collapses — an H13 failure regardless of green tests). Each gate runs three layers: adapter contract conformance, reference/replica differential comparison of normalized outputs, and full constitutional regression against the pinned baseline suite. Directly tests O10 (open to the opposite result: a revealed contract gap bounds what the reference implementation actually proved, recorded honestly as a mixed outcome), O11, O13, and O15. **Status: Untested (0 of 3 adapters).**

> **Hypothesis H14 (registered at AGC Session 023, workstream 4B — Browser and Assistive-Technology Conformance):** Browser rendering and assistive-technology semantics preserve the constitutional presentation constraints established in the static reference surface without introducing visual, focus, or accessibility-based preference.

Repeats the H12 checklist through an actual browser runtime (computed accessibility tree, real focus order, CSS visual order, hidden/clipped content, responsive layout, screen-reader labels) without redesigning the page. **Status: Untested.**

> **Hypothesis H15 (registered at AGC Session 023, workstream 4C — Adversarial and Concurrency Hardening):** Under concurrent, partial-failure, and adversarial conditions, ARGUS preserves constitutional non-effects, provenance, explicit degradation, and refusal semantics without silently repairing or inventing epistemic state.

Systematically attacks the integration boundaries where the program already found failures: audit-head contention, rollback, duplicate submissions, pool reuse, stale/revoked grants mid-request, object-store disappearance during verification, malformed projections, template injection, broken chains, partial infrastructure failure. The goal is *constitutional* resilience: under failure, surface uncertainty and degradation rather than silently normalize, repair, omit, or infer. **Status: Untested.**

> **Hypothesis H16 (registered at AGC Session 023, workstream 4D — Independent Reimplementation Study):** An independent implementation derived from the same ontology and constitutional specifications converges on materially equivalent domain behavior without access to the reference implementation.

The highest-value ODE experiment now available: another team receives only the ontology, ADRs, derivation specification, refusal matrices, and conformance fixtures — never the reference implementation — and the comparison covers domain structures, validators, refusal codes, derived predicates, API behavior, read models, and presentation constraints. This moves H1–H12 beyond internal dual rendering toward genuine independent convergence — the experiment most capable of validating ODE as more than a disciplined internal development method. **Status: Untested.**
...
| 0.19.0 | 2026-07-13 | H10 refined at the Slice 1F-A gate review (AGC Session 018): constitutional command execution across paths, the four-way authentication/attribution/authority/epistemic separation, honest single-implementation authentication bound; candidate O13 (three-layer identity agreement at execution time) registered, deliberately unpromoted. |
| 0.20.0 | 2026-07-13 | H10 marked Supported (n = 1); O10 promoted (infrastructure reuse across two integrations — 3A serializers, 1F-A audit-chain preservation) and O13 promoted (three-layer identity agreement at execution time); candidate O14 (explicit withholding preserves a singular record) registered; Hypothesis H11 (authority/visibility with epistemic neutrality, untested) registered ahead of Slice 1F-B; the authenticated-actor standing limitation marked addressed at the identity layer with the production-IdP limitation added — per AGC Session 019. |
| 0.21.0 | 2026-07-13 | H11 refined at the Slice 1F-B gate review (AGC Session 020): authorized withholding (secrecy is not an existence leak), the dual-rendered authorize decision vs. facts-only provider bound; candidate O15 (least-privilege from normalized capabilities beats roles/inheritance) registered, deliberately unpromoted. |
| 0.22.0 | 2026-07-13 | H11 marked Supported (n = 1) with the provider/persistence qualification; O14 promoted (different projections, singular epistemic reality); O15 first evidence recorded, retained as candidate; O9 strengthened to three integration surfaces via the locking discovery (formally recorded); O12 strengthened, retained; candidate O16 (presentation is epistemically consequential) registered; Hypothesis H12 registered untested ahead of Slice 1F-C — per AGC Session 021. |
| 0.23.0 | 2026-07-13 | H12 refined at the Slice 1F-C gate review (AGC Session 022): the no-new-epistemic-meaning clause, two-class content parity in both directions, accessibility-semantic source parity as the honest testing bound, projection-only rendering, and the red-team scanner obligation. |
| 0.24.0 | 2026-07-13 | Program completion (AGC Session 023): H12 marked Supported (n = 1) with the static-conformance limitation; O16 and O12 promoted with final wordings; O9 annotated at four composition surfaces; the H1–H12 ledger closed at n = 1 each; replication declared the next research priority; Phase 4 hypotheses H13–H16 registered untested (adapters, browser/AT conformance, adversarial hardening, independent reimplementation). |
| 0.25.0 | 2026-07-21 | H13 refined per AGC Session 024 (Resolution 027 / ONT-PRN-032 / ADR-0038): substitutability measured against Baseline 1.0 constitutional behavior, never interface compatibility; umbrella structure with cumulative sub-results H13-S1/S2/S3 behind separate ordered gates; four-category failure taxonomy and three-layer test structure recorded; 4A-1 (MinIO) designated the O11 repeat test, 4A-3 (persisted grants) designated the O15 repeat test. |
