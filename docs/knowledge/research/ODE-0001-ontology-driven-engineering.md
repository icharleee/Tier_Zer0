...
**Standing limitations (publication mandatory, per Sessions 012–013; status updated Session 019):** no evidentiary sufficiency model; no formal comparative assessment; no calibrated confidence; no AI provenance at Hypothesis; ~~no authenticated actor identity~~ **addressed at the constitutional identity layer** by Slice 1F-A (ONT-PRN-028), though **production identity-provider integration remains unimplemented** — an implementation limitation, not a constitutional gap; no end-user review workflow; and — added by Session 013 — no formal mechanism yet distinguishes creation admissibility from later human endorsement or institutional adoption (a warning that future workflow must never smuggle epistemic truth into administrative approval, not permission to build an "accepted hypothesis" state).
...
*Candidate observation O15 (registered at AGC Session 020, deliberately not promoted):* Least-privilege authority is more stable when authorization decisions are derived from normalized capabilities and explicit resource scopes rather than organizational roles or implicit privilege inheritance. Slice 1F-B provides the first evidence; a later persisted grant store or real organizational-role mapping is the stronger repeat test.
